...
import br.com.senac.jogoPokemon.Ataques.AtaqueEspecialStrategy;
import br.com.senac.jogoPokemon.Ataques.AtaqueSimplesStrategy;
import br.com.senac.jogoPokemon.Batalha.BatalhaFacade;
import br.senac.jogopokemon.tipoPokemon.TipoGramaComposite;
import br.senac.jogopokemon.tipoPokemon.TiposSimples;
...
public class Main {
	public static void main(String[] args) {
		...
		// 4 - Composite: esse design funciona aplicando alguns conceitos de heranca, pois podemos criar uma interface que vai se implementada
		//	   em dua classes, uma que sera uma classe unica, e outra classe que contera outras classes subjacente ou ate mesmo listas dentro dela
		//	   
		//	   implementacao: criei uma interface TipoPokemon e seu methodo padrao seria o tipo(Pokemon pokemon) que e implementada em duas
		// 	   classes subjacentes, sendo uma TipoSimples que apenas retorna o tipo do pokemon, e uma TipoGramaComposite, que possui 
		//	   um arrayList dentro dela onde podemos adicionar novas subclasses do pokemon.
		
		// 5 - Facade: serve basicamente como um Controler, e la que serao chamados os methodos para serem executados posteriormente na classe principal
		//     
		//	   implementacao: criei a classe batalhaFacade que serve para apenas executar as funcoes de combate dos pokemons, seja um ataque,
		//     a utilizacao de algum item, uma evolucao.
		
		//Composite: instanciando uma classe TipoSimples para o pokemon possuir apenas um tipo 
		TiposSimples tipoFogo = new TiposSimples("Fogo");
		
		//Composite: instanciando uma classe TipoGramaComposite para que possamos passsar um tipo e subtipos para o pokemon
		...
		Pokemon bulbasauro = new Pokemon(tipoGramaComposto, 100, 20, 50, 0, new AtaqueEspecialStrategy(), Evolucao.BULBASAURO);

		//Facade: basicamente criei uma classe a parte para apenas poder executar as tarefas da batalha para que o usuario nao saiba como funciona
